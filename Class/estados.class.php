...
if (isset($EstadosInstanciada)) {
	...
	class Estados
	{
		...
		function selectEstados($nomeCampo = 'id_estado', $style = "padding:5px;", $selecionado = '', $campoLabel = 'uf', $chamaCidades = '', $class = 'form-control')
		{
			...
			?>
			<select name="<?php echo $nomeCampo; ?>" id="<?php echo $nomeCampo; ?>" class="<?php echo $class; ?>"
				style=" <?php echo $style; ?>" <?php if ($chamaCidades == 'S') { ?>onChange="AtualizaJanela('cidades.php?id_estado='+this.value, 'Cidades');" <?php } ?>>
				<option value=""></option>
				<?php
				foreach ($estados as $estado) {
					?>
					<option value="<?php echo $estado->id ?>" <?php if (isset($selecionado) && !empty($selecionado) && $estado->id == $selecionado) {
						  echo "selected";
					  } ?>><?php if (isset($estado->uf) && !empty($estado->uf)) {
						   echo $estado->uf;
					   } ?></option>
				<?php
				}
